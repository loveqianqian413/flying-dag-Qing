...
function impact(x0, y0, x1, y1) {
	...
	if (up) {
		if (dx < 14 && dx > 0 && dy > -1) {
			return true;
		} else if (dx < 0 && dx > -50 && dy > -14) {
			return true;
		} else if (dx < -50 && dx > -64 && dy > -1) {
			return true;
		} else if (Math.sqrt(dx * dx + dy * dy) < 14 || Math.sqrt((dx + 50) * (dx + 50) + dy * dy) < 14) {
			return true;
		} else {
			return false;
		}
